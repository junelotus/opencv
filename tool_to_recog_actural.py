...
def getTimeresult(path, deblur_tool, class_tool, imageModel, contours_model_result,name_flag):
    ...
    for i in range(len(listsOfImg)):
        ...
        if result_list_fiften.count(-1) <= 2 and image_to_harr == None:
            # img_to_harr_now = reShape(imageModel, img_to_harr_now)
...
def chose_final_corner_dot_list(cornerdots_list,x_or_y,coordinate_limit_1,coordinate_limit_2,thread_var):
...
def draw_picture(AffineImg,img_to_harr,AffineMatrix,width,height,canvas_point):
    ...
    result_AffineImg =  np.ones((height,width,3),dtype=np.uint8)#img_to_harr[0:height,0:width]
    # cv2.circle(img_to_harr, ( 0,int(img_to_harr.shape[0]-10)), 3, (0, 255, 0), 30)
    # showImg('test',img_to_harr)
    
    # print(np.dot(AffineMatrix,[canvas_point[3][0],canvas_point[3][1],1]))
    for x in range(img_to_harr.shape[0]):
        for y in range(img_to_harr.shape[1]):
            ...
            and  y-l_k*x-l_d  >= 1.0
             and y-r_k*x-r_d <= -1.0):
                ...
                result_AffineImg[dst_x][dst_y] = img_to_harr[x][y]

    return result_AffineImg
...
path_switch = glob.glob('./Archive_to_split_2/AffineImg*.jpg')
length_path = len(path_switch)
for i in range(length_path-1):
    Img1 = cv2.imread(path_switch[i])
    Img2 = cv2.imread(path_switch[i+1])
    Img2 = reShape(Img1,Img2)
    print(path_switch[i],path_switch[i+1],L2Loss(Img1, Img2))
'''
path_switch = glob.glob('./jpg01/*.jpg')
src_point = []
path_second = None
flag_of_path = 40
for path in path_switch:
    ...
    if img_to_harr is None:
        continue
    img_result_1 = cv2.cvtColor(img_to_harr,cv2.COLOR_RGB2GRAY)
    rct, img_result_1 = cv2.threshold(img_result_1, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    ...
    matRotate = cv2.getRotationMatrix2D((img_to_harr.shape[0]*0.5,img_to_harr.shape[1]*0.5),angle,1)    # 为方便旋转后图片能完整展示，所以我们将其缩小
    img_to_harr = cv2.warpAffine(img_to_harr,matRotate,(img_to_harr.shape[1],img_to_harr.shape[0]))
    # showImg('first',img_to_harr) 
    ...
    # min_rect = cv2.minAreaRect(contoursub[0])
    # x, y = min_rect[0]
    # cv2.circle(img_to_harr, (int(x), int(y)), 3, (0, 255, 0), 5)
    # showImg('first',img_to_harr)

    # height = int(min_rect[1][0])
    # width= int(min_rect[1][1])
    # point = np.array(cv2.boxPoints(min_rect)).tolist()
    # dst = [[width,height],[0,height],[0,0],[ width,0],]
    # # dst = [[0,height],[0,0],[ width,0],[width,height]]
    # print(min_rect)
    # print(point)
    # M, mask = cv2.findHomography(np.float32(point).reshape(-1,1,2), np.float32(dst))#, cv2.RANSAC,5.0)
    # img_to_harr = cv2.warpPerspective(img_to_harr, M, (width, height))
    
    # matRotate = cv2.getRotationMatrix2D((img_to_harr.shape[0]*0.5,img_to_harr.shape[1]*0.5),90,1)    # 为方便旋转后图片能完整展示，所以我们将其缩小
    # img_to_harr = cv2.warpAffine(img_to_harr,matRotate,(img_to_harr.shape[0],img_to_harr.shape[1]))
    #
    showImg('AffineImg',img_to_harr)    
    ...
    # img_to_harr = reShape(imageModel, img_to_harr)
    # showImg('now',img_to_harr)   
    ...
    if len(lu_list) > 0 :
        result_list_x,need_y,p1 = chose_final_corner_dot_list(lu_list,'x',0,0,10)
        if need_y :
            result_list_y,need_y,p1 = chose_final_corner_dot_list(lu_list,'y',0,0,10)
            p1 = chose_final_corner_dot(result_list_x,result_list_y)
        cv2.circle(img_to_harr,(int(p1[1]),int(p1[0])),16,(255,255,0))
        canvas_point.append(p1)
    
    if len(lb_list) > 0 :
        result_list_x,need_y,p1 = chose_final_corner_dot_list(lb_list,'x',shape[0],0,10)
        print('************ddddddd',lb_list)
        print('************ddddddd',result_list_x)
        if need_y :
            result_list_y,need_y,p1 = chose_final_corner_dot_list(lb_list,'y',shape[0],0,10)
            print('************ddddddd',result_list_y)
            p1 = chose_final_corner_dot(result_list_x,result_list_y)
        print('************ttttt',p1)    
        cv2.circle(img_to_harr,(int(p1[1]),int(p1[0])),16,(255,0,0))
        canvas_point.append(p1)
    if len(rb_list) > 0:
        result_list_x,need_y,p1 = chose_final_corner_dot_list(rb_list,'x',shape[0],shape[1],10)
        if need_y :
            result_list_y,need_y,p1 = chose_final_corner_dot_list(rb_list,'y',shape[0],shape[1],10)
            p1 = chose_final_corner_dot(result_list_x,result_list_y)
        cv2.circle(img_to_harr,(int(p1[1]),int(p1[0])),16,(0,255,0))
        canvas_point.append(p1)
    if len(ru_list) > 0:
        result_list_x,need_y,p1 = chose_final_corner_dot_list(ru_list,'x',0,shape[1],10)
        if need_y :
            result_list_y,need_y,p1 = chose_final_corner_dot_list(ru_list,'y',0,shape[1],10)
            p1 = chose_final_corner_dot(result_list_x,result_list_y)
        cv2.circle(img_to_harr,(int(p1[1]),int(p1[0])),16,(0,0,255))
        canvas_point.append(p1)
    if len(canvas_point) == 4:
        ...
        # point = np.array(cv2.boxPoints(min_rect)).tolist()
        # dst = [[0,height],[0,0],[ width,0],[width,height]]
        # width = img_to_harr.shape[1]
        # height = img_to_harr.shape[0]
        # dst = np.array([[0,0],[height ,0],[height,width],[0,width]])
        cv2.imencode('.jpg', img_to_harr)[1].tofile('Archive_to_split_2/harr'+str(flag_of_path)+'.jpg')#img        
        ...
        # AffineImg = cv2.warpPerspective(img_to_harr, M, (width, height))
        ...
        # img_to_harr = img_to_harr[a_begin:a_end,b_begin:b_end]
        AffineImg = cv2.warpPerspective(img_to_harr, AffineMatrix,(int( width),int( height)))# (img_to_harr.shape[1], img_to_harr.shape[0]))#

        result_AffineImg = draw_picture(AffineImg,img_to_harr,AffineMatrix,width,height,canvas_point)
        result_AffineImg = reShape(imageModel, result_AffineImg)
        
        # img_result_1 = cv2.cvtColor(AffineImg,cv2.COLOR_RGB2GRAY)
        # rct, img_result_1 = cv2.threshold(img_result_1, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        # contoursub, _ = cv2.findContours(img_result_1, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        # contoursub.sort(key=takeAera, reverse=True)
        ...
        showImg('AffineImg1',result_AffineImg)
        # min_rect = cv2.minAreaRect(contoursub[0])
        # print(min_rect)
        # angle = min_rect[2]
        # if abs(min_rect[2]) >= 45:
        #     angle = 90-abs(min_rect[2])
        # matRotate = cv2.getRotationMatrix2D((AffineImg.shape[0]*0.5,AffineImg.shape[1]*0.5),angle,1)    # 为方便旋转后图片能完整展示，所以我们将其缩小
        # AffineImg1 = cv2.warpAffine(AffineImg,matRotate,(AffineImg.shape[1],AffineImg.shape[0]))
        # showImg('second',AffineImg1) 
        
        print(min_rect)
        cv2.imencode('.jpg', result_AffineImg)[1].tofile('Archive_to_split_2/AffineImg'+str(flag_of_path)+'.jpg')#img        
    # Img1 = cv2.imread('Archive_to_split_2/AffineImg1.jpg')
    # Img2 = cv2.imread('Archive_to_split_2/AffineImg6.jpg')
    # Img2 = reShape(Img1,Img2)
    # print(L2Loss(Img1, Img2))

    # Img1 = cv2.imread('Archive_to_split_2/AffineImg7.jpg')
    # Img2 = cv2.imread('Archive_to_split_2/AffineImg6.jpg')
    # Img2 = reShape(Img1,Img2)
    # print(L2Loss(Img1, Img2))

    # Img = cv2.imread('Archive_to_split_2/harr'+str(flag_of_path-1)+'.jpg')
    # AffineImg = cv2.warpPerspective(Img, AffineMatrix, (Img.shape[1], Img.shape[0]))
    # img_result_1 = cv2.cvtColor(AffineImg,cv2.COLOR_RGB2GRAY)
    # rct, img_result_1 = cv2.threshold(img_result_1, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    ...
    # if len(contoursub)>0:
    #     x,y,w,h = cv2.boundingRect(contoursub[0])
    #     AffineImg = AffineImg[y:y+h, x:x+w]
    #     Img = Img[y:y+h, x:x+w]
    # showImg('AffineImg',AffineImg)
    # cv2.imencode('.jpg', AffineImg)[1].tofile('Archive_to_split_2/AffineImg_2.jpg')#img        

    # Img = cv2.imread('Archive_to_split_2/harr1.jpg')
    # AffineImg = Img#cv2.warpAffine(Img, AffineMatrix, (Img.shape[1], Img.shape[0]))
    # img_result_1 = cv2.cvtColor(AffineImg,cv2.COLOR_RGB2GRAY)
    # rct, img_result_1 = cv2.threshold(img_result_1, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    ...
    # cv2.imencode('.jpg', Img)[1].tofile('Archive_to_split_2/harr3.jpg')#img        
